...
void drivePD(double targetDistance) {
  ...
  mainControl.Screen.print("Distance %f", currentDistance);
...
void drivePDSlow(double targetDistance) {
  // Brain.Screen.print("HEY BESTIE UWU");
  double motorspeed = 0, currentDistance = 0,
         error = (targetDistance - currentDistance), errorSum = 0, deltaE = 0,
         lastError = 0, maxAllowedError = 1.0, errorTimerMax = 50;
  double kP = 3, kI = 0.1, kD = 0; // 15, 1/20: 10 0 17 ->5 0.05 0, actually PI again lmao kp=5,ki=0.05,kd=0 trying kp.08
  bool timerExpired = false;
  timer errorTimer = timer();
  errorTimer.clear();
  FrontRightMotor.resetRotation(); // used front right motor to control driving distance
...
void drivePDSlower(double targetDistance) {
  // Brain.Screen.print("HEY BESTIE UWU");
  double motorspeed = 0, currentDistance = 0,
         error = (targetDistance - currentDistance), errorSum = 0, deltaE = 0,
         lastError = 0, maxAllowedError = 1.0, errorTimerMax = 50;
  double kP = 2, kI = 0.2, kD = 0; // 15, 1/20: 10 0 17 ->5 0.05 0, actually PI again lmao kp=5,ki=0.05,kd=0 trying kp.08
  bool timerExpired = false;
  timer errorTimer = timer();
  errorTimer.clear();
  FrontRightMotor.resetRotation(); // used front right motor to control driving distance
...
bool BackClampIsClosed;
// Back Clamp Motor Code
void backClampOpen(){
  BackClamp.rotateFor(-125, rotationUnits::deg, 80, velocityUnits::pct, false);
  BackClampIsClosed = false;
}

void backClampClose(){  // Pass air into piston to close
  BackClamp.rotateFor(125, rotationUnits::deg, 80, velocityUnits::pct, false);
  BackClampIsClosed = true;
}

void backClampDrive(){
  if (BackClampIsClosed){
    backClampOpen();
    BackClampIsClosed = false;
  }
  else{
